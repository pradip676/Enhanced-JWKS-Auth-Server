...
class TestProject3JWKS(unittest.TestCase):
    ...
    def test_register_duplicate_user(self):
        ...
        self.client.post("/register", json={"username": "user", "email": "a@a.com"})
        response = self.client.post("/register", json={"username": "user", "email": "b@b.com"})
        self.assertEqual(response.status_code, 409)

    def test_auth_endpoint_success(self):
        """Test /auth returns a JWT for valid user."""
        reg = self.client.post("/register", json={"username": "authuser", "email": "auth@x.com"})
        password = reg.get_json()["password"]
        generate_and_save_keys()

        response = self.client.post("/auth", json={"username": "authuser", "password": password})
        self.assertEqual(response.status_code, 200)
        self.assertIn("token", response.get_json())

    def test_auth_logging(self):
        """Test /auth logs the request to auth_logs."""
        reg = self.client.post("/register", json={"username": "loguser", "email": "log@x.com"})
        password = reg.get_json()["password"]
        generate_and_save_keys()

        self.client.post("/auth", json={"username": "loguser", "password": password})

        with sqlite3.connect(DB_FILE) as conn:
            log_count = conn.execute("SELECT COUNT(*) FROM auth_logs").fetchone()[0]
            self.assertEqual(log_count, 1)

    def test_auth_rate_limit(self):
        """Test /auth rate limiter returns 429 after 10 requests/sec."""
        reg = self.client.post("/register", json={"username": "rate", "email": "rate@x.com"})
        password = reg.get_json()["password"]
        generate_and_save_keys()

        for _ in range(10):
            self.client.post("/auth", json={"username": "rate", "password": password})
        response = self.client.post("/auth", json={"username": "rate", "password": password})
    ...
    def test_auth_invalid_password(self):
        ...
        self.client.post("/register", json={"username": "user1", "email": "u@x.com"})
        generate_and_save_keys()

        response = self.client.post("/auth", json={"username": "user1", "password": "wrongpass"})
